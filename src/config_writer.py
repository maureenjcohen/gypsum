...
#    print(list_out) # Print to check your list formatting matches examples
    ...
    with open(str(path) + 'configfiles/config_%s_%s.txt' %(coords[1], coords[2]), 'w') as file:
        for line in list_out:
            file.write(line)
            
    file.close()
    # Write to a textile file labeled with array column coordinates
    
    return(list_out)
    

def day_generator(path, cubes, day=-1):
    ...
    for latitude in range(0,90):
        east_config = write_config(path, cubes, day, coords=(-1, latitude, 36))
        east_filename = str(path) + 'configfiles/config_%s_36.txt' %(latitude)
        east_cmd = 'curl -d type=all -d whdr=y --data-urlencode file@%s https://psg.gsfc.nasa.gov/api.php' %(east_filename)
        with open(str(path) + 'spectra/trn_%s_36.txt' %(latitude), 'w') as file:
            subprocess.run(east_cmd, shell=True, stdout=file)
        file.close()
        
        west_config = write_config(path, cubes, day, coords=(-1, latitude, 108))
        west_filename = str(path) + 'configfiles/config_%s_108.txt' %(latitude)
        west_cmd = 'curl -d type=all -d whdr=y --data-urlencode file@%s https://psg.gsfc.nasa.gov/api.php' %(west_filename)
        with open(str(path) + 'spectra/trn_%s_108.txt' %(latitude), 'w') as file:
